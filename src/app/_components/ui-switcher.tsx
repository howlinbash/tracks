...
const UiSwitcher = ({ ServerFilterLists, ServerSongList }: SwitcherProps) => {
  const { data: graph, isLoading: graphIsLoading } =
    api.filter.getFilterGraph.useQuery();
  const { data: songs, isLoading: songsIsLoading } =
    api.song.getSongs.useQuery();
